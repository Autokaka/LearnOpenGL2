...
vec4 GetEmissionLightColor() {
  return texture(u_material.emission, v_tex_coord);
}
...
void main() {
  ...
  vec4 emission_color = GetEmissionLightColor();

  FragColor = ambient_color + diffuse_color + specular_color + emission_color;
}
)__SHADER__";
