...
uniform struct Light {
  float inner_cut_off;  // spot light inner cut off angle in cosine value
  float outer_cut_off;  // spot light outer cut off angle in cosine value
  vec3 direction; // spot light shooting direction
...
vec4 GetSpecularLightColor() {
  ...
  return texture(u_material.specular, v_tex_coord) * specular * u_light.specular;
}
...
void main() {
  ...
  float light_coverage = dot(-normalize(u_light.direction), light_direction);
  float intensity = (light_coverage - u_light.outer_cut_off) / (u_light.inner_cut_off - u_light.outer_cut_off);

  FragColor = clamp(intensity, 0.0, 1.0) * (ambient_color + diffuse_color + specular_color);
}
)__SHADER__";
