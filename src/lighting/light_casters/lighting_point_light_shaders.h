...
float GetAttenuation() {
  float distance = length(v_position - u_light.position);
  return 1.0 / (u_light.constant + u_light.linear * distance +
...
void main() {
  ...
  vec4 ambient_color = attenuation * GetAmbientLightColor();
  vec4 diffuse_color = attenuation * GetDiffuseLightColor();
  vec4 specular_color = attenuation * GetSpecularLightColor();
