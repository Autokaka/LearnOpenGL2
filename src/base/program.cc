...
void Program::SetUniformProvider(std::string_view name,
...
SharedShader Program::MakeGLObject() {
  ...
  gl_object_ =
      Shader::CreateFromSource(vertex_shader_.data(), fragment_shader_.data());
  ...
  if (gl_object_) {
    ...
    for (const auto& [name, uniform_provider] : uniform_providers_) {
      ...
      if (IsBool(uniform_value)) {
        ...
        gl_object_->SetVec2(name.data(),
                            std::any_cast<glm::vec2>(uniform_value));
      } else if (IsVec3(uniform_value)) {
        gl_object_->SetVec3(name.data(),
                            std::any_cast<glm::vec3>(uniform_value));
      } else if (IsVec4(uniform_value)) {
        gl_object_->SetVec4(name.data(),
                            std::any_cast<glm::vec4>(uniform_value));
      } else if (IsMat2(uniform_value)) {
        gl_object_->SetMat2(name.data(),
                            std::any_cast<glm::mat2>(uniform_value));
      } else if (IsMat3(uniform_value)) {
        gl_object_->SetMat3(name.data(),
                            std::any_cast<glm::mat3>(uniform_value));
      } else if (IsMat4(uniform_value)) {
        gl_object_->SetMat4(name.data(),
                            std::any_cast<glm::mat4>(uniform_value));
      } else if (IsSampler2D(uniform_value)) {
        gl_object_->SetSampler2D(name.data(),
                                 std::any_cast<SharedTexture>(uniform_value));
      }
    }
  }
}
