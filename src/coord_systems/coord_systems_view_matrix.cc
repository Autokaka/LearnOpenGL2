...
void ProcessInput(GLFWwindow* window) {
  ...
  if (glfwGetKey(window, GLFW_KEY_RIGHT) == GLFW_PRESS) {
    x_offset = std::max(-1.0f, x_offset - 0.1f);
  }
  if (glfwGetKey(window, GLFW_KEY_LEFT) == GLFW_PRESS) {
    x_offset = std::min(1.0f, x_offset + 0.1f);
  }
  if (glfwGetKey(window, GLFW_KEY_DOWN) == GLFW_PRESS) {
    y_offset = std::min(1.0f, y_offset + 0.1f);
  }
  if (glfwGetKey(window, GLFW_KEY_UP) == GLFW_PRESS) {
    y_offset = std::max(-1.0f, y_offset - 0.1f);
  }
  if (glfwGetKey(window, GLFW_KEY_LEFT_SHIFT) == GLFW_PRESS) {
    z_offset = std::min(1.0f, z_offset + 0.1f);
  }
  if (glfwGetKey(window, GLFW_KEY_LEFT_CONTROL) == GLFW_PRESS) {
    z_offset = std::max(-1.0f, z_offset - 0.1f);
  }
}
  ...
  while (!glfwWindowShouldClose(window)) {
    ...
    for (auto&& cube_position : cube_positions) {
      // set uniforms
      {
        ...
        model = glm::rotate(model, rotate_radians, glm::vec3(0.5f, 1.0f, 0.0f));
        shader_program->SetMat4("uModel", model);
        glm::mat4 view = glm::mat4(1.0f);
        view = glm::translate(view, glm::vec3(0.0f + x_offset, 0.0f + y_offset,
                                              -3.0f + z_offset));
