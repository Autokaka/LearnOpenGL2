...
#include "lighting_smooth_spot_light_shaders.h"
...
class MyApp final : public App, AppDelegate {
 public:
  ...
  void WindowWillAppear() override {
    // create VBO
    // clang-format off
    vbo_ = VertexBuffer::Create(cube_vertices, std::vector<Attribute>({
      { "a_position", AttributeType::kVec3 },
      { "a_normal", AttributeType::kVec3 },
      { "a_tex_coord", AttributeType::kVec2 },
    }));
    // clang-format on
  ...
  void DrawContent() override {
    ...
    {  // draw lighted entity
      render_pass_->UseProgram(lighted_shader_);
      {
        ...
        lighted_shader_->SetFloat("u_light.inner_cut_off", glm::cos(glm::radians(light_.inner_cut_off)));
        lighted_shader_->SetFloat("u_light.outer_cut_off", glm::cos(glm::radians(light_.outer_cut_off)));
        lighted_shader_->SetVec3("u_light.direction", light_.direction);
 ...
 private:
  ...
  class PhongLight {
   public:
    float inner_cut_off = 12.5f;       // spot light inner cut off angle in degrees
    float outer_cut_off = 15.0f;       // spot light outer cut off angle in degrees
    glm::vec3 direction{0, 0, -2.0f};  // spot light shooting direction
  ...
  void DrawUI() {
    ...
    ImGui::SliderFloat("light.inner_cut_off", &light_.inner_cut_off, 0.0f, 90.0f);
    ImGui::SliderFloat("light.outer_cut_off", &light_.outer_cut_off, 0.0f, 90.0f);
    light_.inner_cut_off = glm::min(light_.inner_cut_off, light_.outer_cut_off);
